...
for porte in ImageCollection2:
    porte_resize = cv2.resize(porte, (int(porte.shape[1]//10), int(porte.shape[0]//15)))

    porte_filter = ndimage.uniform_filter(porte_resize, size=2)

    if porte_filter.any() < 50:
        porte_filter[porte_filter < 50] = 0
    else:
        porte_filter[porte_filter] = 255
